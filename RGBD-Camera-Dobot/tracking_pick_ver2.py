...
def pick_block(pos, r):
    approach = [pos[0], pos[1], -18, r]
    movejp(approach)
    grip()
    place = [65, 260, -18, 90]
    movejp(place)
    ...
    while True:
        ...
        for i in contours:
            ...
            if area > 1000:
                ...
                angle = rect[2]
                if angle < -45:  # OpenCV가 -90° 쪽으로 넘기는 경우를 0~45°로 변환
                    angle += 90
                # 카메라 기준 각도 → 로봇 기준 각도 변환 (오프셋은 실측)
                R_offset = -12.0  # 직접 한 번 잡아서 맞춘 뒤 기록
                r_target = 90 - angle + R_offset
        ...
        if contours and not picked:
            target_position = [
                ...
                ccx * (410 / 640) - 210,
                -18,
                r_target,  # 이 값은 pick_block의 내부에서 다시 쓰이므로 포함 X
            ]
            pick_block(target_position, r_target)
            picked = True
